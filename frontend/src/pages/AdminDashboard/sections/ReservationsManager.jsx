...
import { Search, Filter, Download, Eye, Edit, CheckCircle, X, Plus } from 'lucide-react';
...
const ReservationsManagement = () => {
  ...
  const getStatusBadgeStyle = (status) => {
    ...
    return `px-3 py-1 rounded-full text-xs font-medium ${styles[status] || 'bg-gray-300 text-gray-700'}`;
  };
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="grid grid-cols-5 gap-4 mb-8">
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border">
          ...
          <p className={`text-3xl font-bold ${getStatusColor('Confirmed')}`}>{statusCounts.confirmed}</p>
        </div>
        <div className="bg-white p-6 rounded-lg shadow-sm border">
          <h3 className="text-gray-600 text-sm font-medium mb-2">Pending</h3>
          <p className={`text-3xl font-bold ${getStatusColor('Pending')}`}>{statusCounts.pending}</p>
        </div>
        <div className="bg-white p-6 rounded-lg shadow-sm border">
          <h3 className="text-gray-600 text-sm font-medium mb-2">Expired</h3>
          <p className={`text-3xl font-bold ${getStatusColor('Expired')}`}>{statusCounts.expired}</p>
        </div>
        <div className="bg-white p-6 rounded-lg shadow-sm border">
          <h3 className="text-gray-600 text-sm font-medium mb-2">Cancelled</h3>
          <p className={`text-3xl font-bold ${getStatusColor('Cancelled')}`}>{statusCounts.cancelled}</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border">
        <div className="p-6">
          <div className="mb-4">
            <h2 className="text-xl font-semibold text-gray-900">Reservation List</h2>
            <p className="text-gray-600">Manage and track all parking reservations</p>
          </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-200">
                  <th className="text-left py-3 px-4 font-medium text-gray-700">Reservation ID</th>
              ...
              <tbody>
                {filteredReservations.map((reservation) => (
                  <tr key={reservation.id} className="border-b border-gray-100 hover:bg-gray-50">
                    <td className="py-4 px-4 font-medium text-gray-900">{reservation.id}</td>
                    ...
                    <td className="py-4 px-4 text-gray-700">{reservation.contact}</td>
                    <td className="py-4 px-4">
                      <div>
                        <div className="font-medium text-gray-900">{reservation.location}</div>
                        <div className="text-sm text-gray-500">{reservation.spot}</div>
                      </div>
                    </td>
                    <td className="py-4 px-4">
                      <div>
                    ...
                    <td className="py-4 px-4 font-medium text-gray-900">{reservation.amount}</td>
                    <td className="py-4 px-4">
