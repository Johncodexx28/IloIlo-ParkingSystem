...
import { Plus, Users, DollarSign, Car, TrendingUp, Eye, Edit, Phone, Mail, Star } from 'lucide-react';
...
const PartnersManagement = () => {
  ...
  const totalSpots = partners.reduce((sum, partner) => sum + partner.properties.spots, 0);
  const avgRating = partners.reduce((sum, partner) => sum + partner.rating, 0) / partners.length;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-white rounded-lg p-6 shadow-sm">
            ...
            <div className="flex items-end justify-between">
              <div>
                ...
                <p className="text-sm text-green-600">+{newPartnersThisMonth} new this month</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg p-6 shadow-sm">
            ...
            <div className="flex items-end justify-between">
              <div>
                <p className="text-3xl font-bold text-gray-900">{formatLargeCurrency(totalRevenue)}</p>
          ...
          <div className="bg-white rounded-lg p-6 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center">
                ...
                <h3 className="text-sm font-medium text-gray-500">Parking Spots</h3>
              </div>
            </div>
            <div className="flex items-end justify-between">
              <div>
                <p className="text-3xl font-bold text-gray-900">{totalSpots.toLocaleString()}</p>
        ...
        <div className="bg-white rounded-lg shadow-sm">
          <div className="px-6 py-4 border-b border-gray-200">
            <h2 className="text-lg font-semibold text-gray-900">Partner Directory</h2>
            <p className="text-sm text-gray-500">Manage partner relationships and performance</p>
          </div>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredPartners.map((partner) => (
                  <tr key={partner.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div>
                        ...
                        <div className="flex items-center mt-1">
                          <span className="text-sm text-gray-500 mr-1">Rating: {partner.rating}</span>
                          <div className="flex text-yellow-400">
                            {[...Array(5)].map((_, i) => (
                              <Star
                                key={i}
                                className={`w-3 h-3 ${
                                  i < Math.floor(partner.rating) ? 'fill-current' : ''
                                }`}
                              />
                            ))}
                          </div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div>
                        <div className="text-sm font-medium text-gray-900">{partner.contact.name}</div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-medium text-gray-900">{partner.commission}%</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span
