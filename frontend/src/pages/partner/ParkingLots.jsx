...
import { Eye, Edit, Settings, Plus, RefreshCw, QrCode, Monitor, Camera, Menu, X } from 'lucide-react';
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="p-4 sm:p-6">
        ...
        <div className="hidden lg:flex justify-between items-center mb-6 xl:mb-8">
          <h1 className="text-2xl xl:text-3xl font-bold text-gray-900">Parking Lots Management</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6 mb-6 sm:mb-8">
          {parkingLots.map((lot) => (
            <div key={lot.id} className="bg-white rounded-lg shadow-sm p-4 sm:p-6 border border-gray-200">
              <div className="flex items-start justify-between mb-4">
                <div className="min-w-0 flex-1">
                  <h3 className="font-semibold text-gray-900 mb-1 text-sm sm:text-base truncate">{lot.name}</h3>
                  <p className="text-xs sm:text-sm text-gray-600 truncate">{lot.location}</p>
                </div>
              ...
              <div className="mb-4">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="text-xs sm:text-sm font-medium">{lot.occupancy.current}/{lot.occupancy.total}</span>
                </div>
              ...
              <div className="grid grid-cols-2 gap-3 sm:gap-4 mb-4 sm:mb-6">
                <div>
                  ...
                  <p className="text-base sm:text-lg font-semibold text-green-600">{lot.available}</p>
                </div>
                <div>
                  <p className="text-xs sm:text-sm text-gray-600">Rate/Hour</p>
                  <p className="text-base sm:text-lg font-semibold">{lot.rate}</p>
                </div>
                <div>
                  <p className="text-xs sm:text-sm text-gray-600">Today's Revenue</p>
                  <p className="text-base sm:text-lg font-semibold">{lot.todayRevenue}</p>
                </div>
                <div>
                  <p className="text-xs sm:text-sm text-gray-600">Peak Hours</p>
                  <p className="text-base sm:text-lg font-semibold">{lot.peakHours}</p>
                </div>
              </div>
        ...
        <div className="space-y-6 lg:space-y-0 lg:grid lg:grid-cols-1 xl:grid-cols-2 lg:gap-8">
          ...
          <div className="bg-white rounded-lg shadow-sm p-4 sm:p-6 border border-gray-200">
            <div className="mb-4 sm:mb-6">
              <h2 className="text-lg sm:text-xl font-semibold text-gray-900 mb-2">Integration Tools</h2>
              <p className="text-sm sm:text-base text-gray-600">QR codes and automated systems for your parking lots</p>
            </div>

            <div className="space-y-3 sm:space-y-4">
              {integrationTools.map((tool) => (
                <div key={tool.id} className="border border-gray-200 rounded-lg p-3 sm:p-4 hover:bg-gray-50 transition-colors">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3 sm:gap-4 min-w-0 flex-1">
                      <div className="p-2 sm:p-3 bg-gray-100 rounded-lg flex-shrink-0">
                        <tool.icon size={20} className="sm:hidden text-gray-600" />
                        <tool.icon size={24} className="hidden sm:block text-gray-600" />
                      </div>
                      <div className="min-w-0 flex-1">
                        <h3 className="font-medium text-gray-900 text-sm sm:text-base truncate">{tool.title}</h3>
                        <p className="text-xs sm:text-sm text-gray-600 truncate">{tool.description}</p>
                      </div>
                    </div>
          ...
          <div className="bg-white rounded-lg shadow-sm p-4 sm:p-6 border border-gray-200">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 sm:mb-6">
              <div className="mb-3 sm:mb-0">
                <h2 className="text-lg sm:text-xl font-semibold text-gray-900 mb-2">Real-time Occupancy</h2>
                <p className="text-sm sm:text-base text-gray-600">Live view of parking spot status</p>
              </div>
              ...
              {parkingLots.map((lot) => (
                <div key={lot.id} className="border-b border-gray-100 last:border-b-0 pb-4 last:pb-0">
                  <div className="flex justify-between items-start sm:items-center mb-2">
                    <h3 className="font-medium text-gray-900 text-sm sm:text-base min-w-0 flex-1 mr-2">{lot.name}</h3>
